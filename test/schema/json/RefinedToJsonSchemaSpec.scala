...
class StackSpec extends PlaySpec {
  ...
  "String Refined NonEmpty" must {
    "return a schema with type String and minLength" in {
      JsonSchema.asJsValue[StringNonEmpty] mustBe Json.parse("""{"d":{"minLength":1,"type":"string"}}""")
    }
  }

  "Collection[String] Refined NonEmpty" must {
    "return a schema with type Collection[String] and minLength" in {
      JsonSchema.asJsValue[CollectionStringNonEmpty] mustBe Json.parse("""{"e":{"minLength":1,"type":"array","items":{"type":"string"}}}""")
    }
  }

  "Collection[Int] " must {
    "return a schema with type Collection[Int] " in {
      JsonSchema.asJsValue[CollectionIntNonEmpty] mustBe Json.parse("""{"ee":{"minLength":1,"type":"array","items":{"type":"integer"}}}""")
    }
  }
...
"Collection[String] with no Refined " must {
  "return a schema with type array " in {
    JsonSchema.asJsValue[CollectionStringNotRefined] mustBe Json.parse("""{"eee":{"type":"array","items":{"type":"string"}}}""")
  }
}

"Collection[Int] with no Refined " must {
  "return a schema with type array " in {
    JsonSchema.asJsValue[CollectionIntNotRefined] mustBe Json.parse("""{"eee":{"type":"array","items":{"type":"integer"}}}""")
  }
}

  "String Refined IPv4" must {
    "return a schema with type String and format = ipv4" in {
      JsonSchema.asJsValue[StringFormatIPV4] mustBe Json.parse("""{"f":{"format":"ipv4","type":"string"}}""")
    }
  }

  "String Refined IPv6" must {
    "return a schema with type String and format = ipv6" in {
      JsonSchema.asJsValue[StringFormatIPV6] mustBe Json.parse("""{"g":{"format":"ipv6","type":"string"}}""")
    }
  }

  "String Refined URI" must {
    "return a schema with type String and format = uri" in {
      JsonSchema.asJsValue[StringFormatURI] mustBe Json.parse("""{"h":{"format":"uri","type":"string"}}""")
    }
  }

  "case class with multiples fields" must {
    "return a schema with all the fields" in {
      JsonSchema.asJsValue[ClassWithTwoFields] mustBe Json.parse("""{"i":{"minLength":1,"type":"string"},"j":{"minValue":1,"type":"integer"}}""")
    }
  }

  "String with min size" must {
    "return a schema with min size" in {
      JsonSchema.asJsValue[StringWithMinSizeZero] mustBe Json.parse("""{"k":{"minLength":0,"type":"string"}}""")
    }  
  }
  ...
  "String with min size 22" must {
    "return a schema with min size" in {
      JsonSchema.asJsValue[StringWithMinSize22] mustBe Json.parse("""{"k":{"minLength":22,"type":"string"}}""")
    }  
  }
  
  "String with max size 22" must {
    "return a schema with max size" in {
      JsonSchema.asJsValue[StringWithMaxSize22] mustBe Json.parse("""{"l":{"type":"string","maxLength":22}}""")
    }  
  }

 "String with min and max size" must {
    "return a schema with min and max size" in {
      JsonSchema.asJsValue[StringWithMinMax] mustBe Json.parse("""{"m":{"minLength":0,"type":"string","maxLength":5}}""")
    }  
  }

  "sealed trait with object" must {
    "return enum with all value as a string" in {
      JsonSchema.asJsValue[SimpleEnum] mustBe Json.parse("""{"e":{"enum":["red","orange","green"],"type":"string"}}""")
    }  
  }
