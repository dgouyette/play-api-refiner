...
object JsonSchema {
  ...
  def openAPI(c: scala.reflect.macros.whitebox.Context)(types : List[(Route,c.universe.Type)]) = {
    import c.universe._
    val json = types.map(t => Json.obj(s"/${t._1.path.toString()}" -> Json.obj(t._1.verb.value.toLowerCase() -> Json.obj("parameters" -> buildOpenAPI(c)(c.WeakTypeTag(t._2)))))).reduce(_ deepMerge _)
    implicit val l: c.universe.Liftable[JsObject] = Liftable((in: JsObject) => q"play.api.libs.json.Json.parse(${in.toString()})")
  ...
  private def buildOpenAPI[T](c: scala.reflect.macros.whitebox.Context)(implicit tag: c.WeakTypeTag[T]) = {
    import c.universe._
